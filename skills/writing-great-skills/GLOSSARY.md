...
The terms are grouped by axis: **Invocation** (how a skill is reached), **Information Hierarchy** (how its content is arranged), **Steering** (how the agent's runtime behaviour is shaped), **Calibration & Testing** (how a skill is fitted to the model that runs it, and how claims about it are settled), and **Pruning** (how it is kept lean). Each **failure mode** lives beside the lever that cures it, tagged _failure mode_.
...
A concrete input-and-output shown in the skill rather than described — the strongest behavioural anchor per token a skill can carry, and an indiscriminate one: the model generalises the example's incidental details along with its point (**overfitting**). Reach for one when prose keeps needing another qualifying clause — the signal that the behaviour is cheaper shown than described. Strip incidentals until everything left _is_ the point. A lone positive example cannot locate a boundary; when the guarded failure is "close but wrong", pair it with a near-miss and name the difference (a _contrasting pair_).

_Avoid_: sample, demo, illustration
...
How a skill is fitted to the model that runs it, and how claims about it are settled.

### Degrees of Freedom

How much of the _how_ a skill leaves to the model — the dial that matches specificity to fragility. Script exactly where one deviation breaks the work (an exact command, a wire format, an order that matters); state goals and invariants where many hows succeed. Set per passage, not per skill: one skill can pin a wire format and free the prose around it. Every freedom revoked is bought with tokens and risks **over-specification**; when unsure, leave the freedom in and let the **testing loop** show which to revoke.

_Avoid_: flexibility, strictness, level of detail

### Over-Specification

_Failure mode._ Specificity pinned past the task's fragility — rules written to constrain a weaker model, obeyed literally by a stronger one in cases the author never foresaw. Distinct from a **no-op**, which changes nothing: an over-specified line changes behaviour in the wrong direction, so the skill can underperform no skill at all. Model-relative like the no-op but its inverse pole: raising model strength turns weak-model guardrails from harmless into harmful. The cure is restoring **degrees of freedom** — keep the script only where deviation actually breaks something.

_Avoid_: micromanagement, over-constraining, rigidity

### Contradiction

_Failure mode._ Two live instructions that cannot both be followed — within a skill, or between the skill and anything loaded beside it (the system prompt, `CLAUDE.md`, another skill). The model resolves it silently, and which line wins varies by run — a direct **predictability** leak, and sharper on stronger models, which reliably notice both lines. The cure is **single source of truth** extended across contexts: a new line is checked not just against the skill it joins but against everything that will be loaded with it.

_Avoid_: conflict, inconsistency, tension

### Overfitting

_Failure mode._ The model generalising a **worked example**'s incidental details along with its point — copying the sample commit's phrasing, not just its format. The example-level cousin of **over-specification**: both are the model complying harder than intended. Cures: strip incidentals until everything left is the point, and use a contrasting pair when a lone positive example cannot locate the boundary.

_Avoid_: anchoring, literalism, copying

### Testing Loop

The empirical cycle that confirms a skill does what its prose claims — a skill is a hypothesis about model behaviour, and prose review cannot confirm it. Run the skill in a fresh context on a real task; diff the observed _process_ against the intended one (output variance is fine, process variance is the bug); trace each divergence to the sentence that allowed it; edit that one sentence; rerun. One variable per rerun, or the fix cannot be attributed. Also the court of appeal for every model-relative judgement — **no-op** verdicts, pointer reliability, **overfitting** — settled by running, not debate.

_Avoid_: eval, QA, validation

### Verdict Pass

The audit's core move: every sentence in every file of a skill receives exactly one verdict — _keep_, _delete_, _collapse_, _move_, or _merge_ — graded against the strongest model that will run the skill. Its **completion criterion** is exhaustive by construction: done means no sentence unverdicted, which is what makes an audit resistant to the "looks fine" skim (**premature completion** in reference clothing).

_Avoid_: line review, sweep
...
A leading word is a _technique_; No-Op is a _verdict_ on a line — and they cross. A leading word too weak to beat the default is a no-op (_be thorough_ when the agent is already thorough-ish), and the fix is a stronger word that passes the verdict (_relentless_), not a different technique. So the No-Op test — does it change behaviour versus the default? — is also how you grade whether a leading word is earning its repetitions. This is model-relative twice over: the default rises with model strength, so grade against the strongest model you run; and two people disagreeing over whether a line is a no-op disagree about the default, and settle it with the **testing loop**, not by debate.

_Avoid_: redundant instruction, restating the obvious, belaboring

## Credits

This glossary extends the one from [`writing-great-skills`](https://github.com/mattpocock/skills/blob/main/skills/productivity/writing-great-skills/SKILL.md) by **Matt Pocock**, which defined every term outside the Calibration & Testing axis (plus **Worked Example**).
